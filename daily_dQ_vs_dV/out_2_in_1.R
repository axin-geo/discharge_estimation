...
y <- excel_sheets("C:/Users/axin/OneDrive - Kansas State University/SWOT_from_Aote/raw_data_by_num_of_streams/out_substraction/DWORSHAK RES._13_14.xlsx") %>% 
  map(~read_xlsx("C:/Users/axin/OneDrive - Kansas State University/SWOT_from_Aote/raw_data_by_num_of_streams/out_substraction/DWORSHAK RES._13_14.xlsx",.)) %>%
  data.frame()

##tidy data
n = c("V", "Q_out1", "Q_out2", "Q_in1")
m = c("site_V", "site_out1", "site_out2", "site_in1")

tidy <- function(s){ 
  s <- s %>%
    select(., datetime, contains("site_no"), ends_with(c("32400","30800","31600")), contains("00003"),-contains("cd")) %>%
    filter(., site_no != "15s") %>%
...
y$Q_out1 <- as.numeric(y$Q_out1) * convert_cfs_mcmd; 
y$Q_out2 <- as.numeric(y$Q_out2) * convert_cfs_mcmd; 
...
y <- y %>%
  mutate(.,dQ = Q_in1 - Q_out1 +Q_out2) %>% cbind(., dV)
